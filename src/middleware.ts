...
export const config = {
  ...
  // (autenticada por Bearer CRON_SECRET na própria rota), /api/webhooks
  // (autenticada por token na própria rota), estáticos
  matcher: ['/((?!api/auth|api/cron|api/webhooks|login|_next/static|_next/image|favicon.ico).*)'],
}
